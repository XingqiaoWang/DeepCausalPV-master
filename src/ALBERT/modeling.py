...
class AlbertModel(object):
  ...
  """
    ...
    """
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            with tf.variable_scope("encoder"):
                ...
                self.all_attention_probabilities, self.all_encoder_layers = transformer_model(
    ...
    def get_attention_probability(self):
        return self.all_attention_probabilities
...
def dot_product_attention(q, k, v, bias, dropout_rate=0.0):
  ...
  """
    ...
    attention_probs = tf.nn.softmax(logits, name="attention_probs") #Tensor with shape [..., length_q, depth_kv].[B,N,F,T]
    attention_probs = dropout(attention_probs, dropout_rate)
    return attention_probs, tf.matmul(attention_probs, v)
...
def attention_layer(from_tensor,
  ...
  """
    ...
    attention_probs, new_embeddings = dot_product_attention(q, k, v, attention_mask,
                                           attention_probs_dropout_prob)
    # 'attention_probs = [B, N, F, T]'
    return attention_probs, tf.transpose(new_embeddings, [0, 2, 1, 3])
  ...
  """
    ...
    with tf.variable_scope("attention_1"):
        with tf.variable_scope("self"):
            attention_probs, attention_output = attention_layer(
    ...
    return attention_probs, ffn_output
                ...
                with tf.name_scope("layer_%d" % layer_idx):
                    ...
                    for inner_group_idx in range(inner_group_num):
                        with tf.variable_scope("inner_group_%d" % inner_group_idx):
                            attention_probs, layer_output = attention_ffn_block(
                            ...
                            all_layer_attention.append(attention_probs)
    if do_return_all_layers:
        return all_layer_attention, all_layer_outputs
    else:
        return all_layer_attention[-1], all_layer_outputs[-1]
