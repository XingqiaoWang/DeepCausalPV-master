...
class Causal_Processor(DataProcessor):
  """Processor for the covid_19 data set."""

  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "proc", "train.tsv")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "proc", "dev.tsv")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "proc", "all.tsv")), "test")
...
def file_based_convert_examples_to_features(
    examples, label_list, max_seq_length, tokenizer, output_file, task_name,sublist=None):
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  def input_fn(params):
    ...
    if not slist==None:
      sublist = tf.data.Dataset.from_tensor_slices(tf.cast(slist, tf.int64))
      d=tf.data.experimental.choose_from_datasets(d, sublist)
    if is_training:
      d = d.repeat()
...
# def sparse_cost_sensitive_loss(logits, labels, cost_matrix=tf.constant(
#         np.array([[0, 1], [9, 0]]),
#         dtype=tf.float32)):
  ...
  with tf.variable_scope("loss"):
    ...
    if task_name != "sts-b":
      if cost_sensitive_loss:
        ...
        batch_cost_matrix = tf.nn.embedding_lookup(cost_matrix, labels)
        eps = 1e-6
        p = tf.clip_by_value(tf.nn.softmax(logits), eps, 1 - eps)
        cost_values = tf.log(1 - p) * batch_cost_matrix
        per_example_loss = tf.reduce_mean(-tf.reduce_sum(cost_values, axis=1))
        probabilities = tf.nn.softmax(logits, axis=-1)
        predictions = tf.argmax(probabilities, axis=-1, output_type=tf.int32)
        # one_hot_labels = tf.one_hot(labels, depth=num_labels, dtype=tf.int32)
        # y=tf.nn.embedding_lookup(
        #   cost_matrix, one_hot_labels
        # )
        # per_example_loss = - tf.log(1 - tf.nn.softmax(logits)) \
        #        * y
